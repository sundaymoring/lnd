...
const MaxTokenIdStringSize = TokenIdSize * 2
...
func (msg *MsgTx) BtcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	if err := binarySerializer.PutUint32(w, littleEndian, msg.Time); err != nil {
		return err
	}
...
func readTxOut(r io.Reader, pver uint32, version int32, to *TxOut) error {
	...
	if to.PkScript, err = readScript(r, pver, MaxMessagePayload,
		"transaction output public key script"); err != nil {
			return err
	}

	if _, err := io.ReadFull(r, to.TokenId[:]); err != nil {
		return err
	}

	return readElement(r, &to.TokenValue)
}
...
func WriteTxOut(w io.Writer, pver uint32, version int32, to *TxOut) error {
	...
	if _, err := w.Write(to.TokenId[:]); err != nil {
		return err
	}

	return binarySerializer.PutUint64(w, littleEndian, uint64(to.TokenValue))
}
