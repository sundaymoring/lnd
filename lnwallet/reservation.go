...
func NewChannelReservation(capacity, fundingAmt btcutil.Amount,
	...
	tokenId wire.TokenId, tokenCapacity, fundingTokenAmt btcutil.Amount,
	pushTokenMSat lnwire.MilliSatoshi, fundingTime uint32) (*ChannelReservation, error) {
	...
	tokenFundingMSat := lnwire.NewMSatFromSatoshis(fundingTokenAmt)
	tokenCapacityMSat := lnwire.NewMSatFromSatoshis(tokenCapacity)
	...
	if fundingAmt == 0 {
		ourBalance = pushMSat
		theirBalance = capacityMSat - pushMSat - feeMSat

		ourTokenBalance = pushTokenMSat
		theirTokenBalance =tokenCapacityMSat - ourTokenBalance
		...
		if capacity == fundingAmt {
			...
			ourBalance = capacityMSat - pushMSat - feeMSat
			theirBalance = pushMSat

			ourTokenBalance = tokenCapacityMSat - pushTokenMSat
			...
			ourTokenBalance = tokenFundingMSat
			theirTokenBalance = tokenCapacityMSat - tokenFundingMSat + pushTokenMSat
		}
...
func (r *ChannelReservation) CommitConstraints(c *channeldb.ChannelConstraints) error {
	...
	if r.partialState.TokenId != wire.EmptyTokenId {
		maxChanReserve = r.partialState.TokenCapacity / 5
	}
